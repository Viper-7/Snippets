...
<?php
	...
	$result = mysql_query("SELECT Filename, Ticket, Quality, Timestamp, Running, Resolution FROM flvTickets ORDER BY Quality, Timestamp");
	...
	while(list($filename,$ticket,$quality,$timestamp,$running,$resolution) = mysql_fetch_array($result)) {
		...
		if($quality=='mobile') {
			echo '<TD><A HREF="/flowplayer?ticket=' . $ticket . '" target=_blank>' . $ticket . '.3gp' . '</A></TD>';
			...
			echo '<TD><A HREF="/flowplayer?ticket=' . $ticket . '" target=_blank>' . $ticket . '.flv' . '</A></TD>';
			echo '<TD>' . ucfirst(trim($quality)) . '</TD>';
			echo '<TD><center>' . $resolution . '</center></TD>';
			echo '<TD><center>' . get_filesize(@filesize($tmpfolder . '/' . $ticket . '.flv')) . '</center></TD>';
		}
		echo '<TD><center>' . format_date($timestamp, 'datetime') . '</center></TD>';
