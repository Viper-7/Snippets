<?php
	...
	// Allocate a unique ticket number
	...
	if (@mysql_num_rows($result) == 0) {
		...
		// Update the browsers data cache on the movie
		if($quality == 'mobile') {
			$dim = '176x144';
		} else {
		...
		$count = 0;
		while ($size < $minsize && $size != $lastsize && $count < 20) {
			$lastsize = $size;
			clearstatcache();
			$size = filesize($tmpfolder . $path);
			$count += 1;
