...
<HTML><HEAD><TITLE>Cartoon Thumbnails</TITLE>
...
<?php	
	...
	$result = mysql_query("SELECT toons.ID, Name, Folder, Summary, IMDBURL, ImageURL, count(tag) as pop FROM toons left join toontags on toons.id=toontags.toonsid GROUP BY toons.id ORDER BY pop DESC LIMIT $start, " . ($cells+1));
	...
	echo ("<BR/><div class='balloon'><ul>\n");

	$count = 0;
	while (list($id, $name, $folder, $summary, $imdburl, $boxurl, $pop) = mysql_fetch_array($result)) {
		$count++;
	...
	echo("<TR><TD COLSPAN=3><BR/><center><font color='#333333'>Page generated in $totaltime seconds.</font>");
?>
