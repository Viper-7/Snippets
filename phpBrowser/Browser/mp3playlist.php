	...
	if ($_GET["dir"] . $_GET["symlink"]) {
		...
		foreach ($contents as $entry) {
			...
			if (filesize($newdir . "/" . $entry) > 307200) {
				...
				$output .= "File$count=http://$serverurl/mp3/?file=" . urlencode($newdir . "/" . $entry) . "\n";
				$title = explode(".", $entry);
				$title = $title[0];
				
				$output .= "Title$count=$title\n";
				$output .= "Length$count=";
				$output .= $mp3->info["length"] ? $mp3->info["length"] : '-1';
				$output .= "\n\n";
			}
			$count += 1;
		}
		...
		$output .= "NumberOfEntries=1\n";
		$output .= "\n";
		$output .= "File1=http://$serverurl/mp3/?file=" . urlencode($name) . "\n";
		$output .= "Title1=$name\n";
		$output .= "Length1=-1\n";
		$output .= "\n";
		$output .= "Version=2\n";
