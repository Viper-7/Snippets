...
<?php
	// Read a list of all flv files in the cache folder
	$files = split("\n", shell_exec('ls ' . $tmpfolder . '/*.flv 2>/dev/null') . "\n" . shell_exec('ls ' . $tmpfolder . '/publish/*.3gp 2>/dev/null'));
	...
	while(list($filename,$ticket,$quality,$timestamp,$running,$resolution) = mysql_fetch_array($result)) {
		...
		if($quality == 'mobile') {
			if($count==1){
				echo '<TD><A HREF="http://viper-7.com/3gp">' . $ticket . '.3gp' . '</A></TD>';
			} else {
				echo '<TD><A HREF="/3gp?ticket=' . $ticket . '">' . $ticket . '.3gp' . '</A></TD>';
			}
			echo '<TD>' . ucfirst(trim($quality)) . '</TD>';
			echo '<TD><center>' . $resolution . '</center></TD>';
			echo '<TD><center>' . get_filesize(@filesize($tmpfolder . '/publish/' . $ticket . '.3gp')) . '</center></TD>';
		} else {
			if($count==1){
				echo '<TD><A HREF="http://viper-7.com/flv">' . $ticket . '.flv' . '</A></TD>';
			} else {
				echo '<TD><A HREF="/flv?ticket=' . $ticket . '">' . $ticket . '.flv' . '</A></TD>';
			}
			echo '<TD>' . ucfirst(trim($quality)) . '</TD>';
			echo '<TD><center>' . $resolution . '</center></TD>';
