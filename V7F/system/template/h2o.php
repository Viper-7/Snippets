...
require H2O_ROOT.'h2o/v7fextensions.php';
...
class H2o {
    ...
    function parse($source, $filename = '', $env = null) {
        ...
        if (!class_exists('V7F\\Template\\H2o_Parser'))
    ...
    static function addTag($tag, $class = null) {
	    ...
	    $tagClass = __NAMESPACE__ . '\\' . $tagClass;
    ...
    static function addFilter($filter, $callback = null) {
        if (is_array($filter)) {
            ...
            return true;
        } elseif (is_string($filter) && class_exists(__NAMESPACE__ . '\\' . $filter) && is_subclass_of(__NAMESPACE__ . '\\' . $filter, __NAMESPACE__ . '\\' . 'FilterCollection')) {
            foreach (get_class_methods(__NAMESPACE__ . '\\' . $filter) as $f) {
                if (is_callable(array(__NAMESPACE__ . '\\' . $filter, $f)))
                    self::$filters[$f] = array(__NAMESPACE__ . '\\' . $filter, $f);
            }
            return true;
        }
        if (is_null($callback))
